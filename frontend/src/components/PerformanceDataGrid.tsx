...
export function PerformanceDataGrid({ series }: PerformanceDataGridProps) {
  ...
  const useDetailedData =
    ...
    typeof series[0].data[0][1] === "object" &&
    series[0].data[0][1] !== null;
    ...
    if (useDetailedData) {
      ...
      serie.data.forEach(([date, value]) => {
        if (typeof value === "object" && value !== null) {
          dataMap.set(date, value as DetailedDataPoint);
        }
      });
  ...
  const columns: GridColDef[] = [
    ...
    ...sortedDates.map((date) => ({
      ...
      valueFormatter: useDetailedData
        ? undefined
        : (value: number | string) => {
            if (typeof value === "number") {
      ...
      renderCell: (params: GridRenderCellParams) => {
        if (useDetailedData && typeof params.value === "object" && params.value !== null) {
          ...
          return <Box sx={{ color: params.row.color, fontWeight: "bold" }}>{percentFormatter(params.value)}</Box>;
        }
        return params.value;
      },
    })),
